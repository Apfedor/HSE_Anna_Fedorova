...
def litigant(i: int):
    print('Введите данные участника спора №' + str(i+1) + ': ')
    myPromt = ['name', 'status', 'inn']
    d = {myPromt[i]: input(myPromt[i] + ': ') for i in range(3)}
    return d


def example4(n: int):
    myList = [litigant(i) for i in range(n)]
    print(myList)
    pass


example4(3) #количество участников спора
